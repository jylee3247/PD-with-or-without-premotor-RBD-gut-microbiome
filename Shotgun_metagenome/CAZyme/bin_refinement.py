...
def dastool_fasta_to_contig2bin(out_dir, *args):
...
def run_dastool(contigs2bin, input_contig, out_path, n_threads):
    ...
    try:
        ...
        raise Exception(f"DAS_Tool error occurred: {e.stderr.decode()}")

if __name__ == "__main__":
    # Arguments
    parser = argparse.ArgumentParser(description='Run DAS_Tool for bin refinement')
    parser.add_argument("-b", '--bin_dir', nargs='+', type=str, required=True, help='Directories containing bins')
    parser.add_argument("-c", '--input_contig', type=str, required=True, help='Path to input contig file')
    parser.add_argument("-o", '--out_dir', type=str, required=True, help='Output directory for DAS_Tool and contigs2bin mappings')
    ...
    for bin_dir in args.bin_dir:
        dastool_fasta_to_contig2bin(args.out_dir, bin_dir)
        binner, ext = find_binner_and_extension(bin_dir, ["metabat2", "maxbin2", "concoct"])
